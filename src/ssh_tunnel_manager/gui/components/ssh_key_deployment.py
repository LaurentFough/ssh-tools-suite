...
class SSHKeyDeployWorker(QThread):
    ...
    def _validate_public_key(self, key_content: str) -> bool:
        ...
        # maxsplit=2 caps this at 3 parts (type, key_data, comment) instead of
        # splitting the comment itself apart - SSH key comments routinely contain
        # spaces (e.g. this app's own default "SSH key generated by SSH Tunnel
        # Manager"), and a plain .split() would blow the old ">3 parts" check on
        # every one of them.
        parts = key_content.split(None, 2)
        if len(parts) < 2:
            return False
    ...
    def _try_ssh_copy_id(self) -> bool:
        ...
        try:
            ...
            # Execute with password script. ssh only invokes SSH_ASKPASS when it has no
            # DISPLAY or no controlling terminal (unless SSH_ASKPASS_REQUIRE=force) - this
            # process inherits both a DISPLAY (we set one below for pre-8.4 OpenSSH) and the
            # GUI's own controlling terminal, so without the fixes below ssh falls straight
            # through to prompting on that terminal instead of using our askpass script.
            env = os.environ.copy()
            env['SSH_ASKPASS'] = script_path
            env['SSH_ASKPASS_REQUIRE'] = 'force'  # OpenSSH >= 8.4: always use askpass
            env.setdefault('DISPLAY', ':0')  # Fallback for pre-8.4 OpenSSH's DISPLAY check

            result = subprocess.run(
                cmd, capture_output=True, text=True, env=env, timeout=30,
                stdin=subprocess.DEVNULL,  # no stdin to prompt on
                start_new_session=True,    # detach from the controlling terminal entirely
            )
...
class SSHConnectionTestWorker(QThread):
    """Worker thread that attempts an actual SSH connection, off the UI thread."""
    ...
    def run(self):
        try:
            ...
            self.test_result.emit(
                False, "Cannot test the connection: the 'paramiko' library is not installed."
            )
            return
        ...
        try:
            ...
            self.test_result.emit(True, f"Successfully connected to {self.username}@{self.host}:{self.port}.")
        except paramiko.AuthenticationException:
            self.test_result.emit(False, "Authentication failed - check the username and password.")
        except paramiko.SSHException as e:
            self.test_result.emit(False, f"SSH connection error: {e}")
        except OSError as e:
            # Covers socket.timeout, connection refused, name resolution failures, etc.
            self.test_result.emit(False, f"Could not reach {self.host}:{self.port} - {e}")
        except Exception as e:
            safe_error = str(e).replace(self.password, '[PASSWORD]')
            self.test_result.emit(False, f"Connection test failed: {safe_error}")
        finally:
            ssh.close()
...
class SSHKeyDeploymentDialog(QDialog):
    ...
    def _test_connection(self):
        """Test SSH connection to server (actually attempts to connect, off the UI thread)."""
        ...
        self.test_worker = SSHConnectionTestWorker(host, port, username, password)
        self.test_worker.test_result.connect(self._handle_test_result)
    ...
    def _handle_test_result(self, success: bool, message: str):
        ...
        if success:
            self.status_label.setText("✅ Connection test succeeded")
